...
def main():
    ...
    if args.wandb_project is not None:
        ...
        # Kurven pro Epoche loggen wir mit step=ep
        wandb.define_metric("train/epoch_error", step_metric="train/epoch")
        wandb.define_metric("train/epoch_rmse_vendor", step_metric="train/epoch")
        wandb.define_metric("train/proxy_reward_vendor", step_metric="train/epoch")
        wandb.define_metric("train/epoch_error", step_metric="train/epoch")
        wandb.define_metric("train/epoch_rmse_vendor", step_metric="train/epoch")
        wandb.define_metric("train/proxy_reward_vendor", step_metric="train/epoch")
    ...
    if wandb_run is not None:
        if err_curve is not None:
            for ep, e in enumerate(err_curve[: args.epochs], start=1):
                wandb.log({"train/epoch": ep, "train/epoch_error": float(e)}, step=ep)

        if rmse_curve is not None:
            for ep, r in enumerate(rmse_curve[: args.epochs], start=1):
