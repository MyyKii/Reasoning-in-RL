...
class ANFIS:
    ...
    def trainHybridJangOffLine(
        ...
        while (epoch < epochs) and (not convergence):
            ...
            rmse = np.sqrt(error / self.Y.size)
            ...
            print("k:", k, "sum_t:", float(np.sum(t)), "eta:", float(eta), flush=True)
...
def forwardHalfPass(ANFISObj, Xs):
    ...
    for pattern in range(len(Xs[:, 0])):
        ...
        #layerTwo = np.array([np.product(x) for x in miAlloc]).T  #in case of numpy >= 1.20
